...
都是在计算有多少种排列组合的方式可以满足条件，"计数"是返回排列组合的数量，"最值"是比较排列组合的出最优解，"存在性"是判断排列组合的数量是否为 0

而求排列组合最暴力的方法就是穷举，既然是穷举，那么就可以看是否存在重叠子问题、最优子结构等特性，判断是否可以用动态规划。
...
1. dp 数组（二维）：`dp[i][j]` 代表了从前 i 个物品中任取，将其放入容量为 j 的背包后的**最大价值**
2. 递推公式：`dp[i][j]` 是最大价值，
...
说明：`weight = [1, 3, 4], value = [15, 20, 30]`
...
当题目满足动态规划的特征，且求的答案是数组的子数组、或来源于子数组。

子数组最重要的是**子数组的元素在原数组中的连续性**。
